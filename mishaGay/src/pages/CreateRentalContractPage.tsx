...
export const CreateRentalContractPage: FC = () => {
  ...
  // ── Категории и клиенты — из Dexie реактивно ────────────────────────────────
  //
  // useLiveQuery обновляет список при каждом изменении в IndexedDB
  // (например, после фонового pull или создания нового клиента офлайн).
  //
  ...
  const clientFilterOption = (
    option: { label: string; value: string | number; data: any },
    inputValue: string
  ) => {
    if (!inputValue) return true;
    if (option.label.toLowerCase().includes(inputValue.toLowerCase())) return true;
    const client = allClients.find((c) => c.id === option.value);
    if (client) {
            ...
            {selectedTools.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
                {selectedTools.map((t) => (
                  <div
                    key={t.id}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#F8FAFC", borderRadius: 8, border: "1px solid #E2E8F0" }}
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "#64748B" }}>
                        ИНВ: {t.inventoryNumber} | {t.dailyRentalPrice ?? t.dailyPrice} с/сутки
                      </div>
                    </div>
                    <button
